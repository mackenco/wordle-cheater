...
export default function WordleSolver() {
  ...
  const excludedLetters = useMemo(() => {
    return Object.keys(letterStates).filter(letter => letterStates[letter] === 'excluded').join('');
  }, [letterStates]);

  const includedLetters = useMemo(() => {
    return Object.keys(letterStates).filter(letter => letterStates[letter] === 'included').join('');
  }, [letterStates]);
  ...
  const filteredWords = useMemo(() => {
    ...
    const excluded = excludedLetters.toLowerCase().split('').filter(l => l.match(/[a-z]/));
    const included = includedLetters.toLowerCase().split('').filter(l => l.match(/[a-z]/));
    ...
    return wordList.filter(word => {
      ...
      if (included.length > 0 && !included.every(letter => word.includes(letter))) return false;
  ...
  const handlePatternChange = (text: string) => {
    ...
    const cleaned = text.toLowerCase().replace(/[^a-z?]/g, '').slice(0, 5);
    setPattern(cleaned);
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        <Text style={styles.title}>Wordle Solver</Text>
        <Text style={styles.subtitle}>Find possible words for your Wordle puzzle</Text>
      </View>

      <View style={styles.inputSection}>
        <View style={styles.inputGroup}>
          <Text style={styles.label}>Word Pattern</Text>
          <Text style={styles.hint}>Use ? for unknown letters (e.g., ?o??y)</Text>
            ...
            <View style={styles.patternDisplay}>
              {Array.from({ length: 5 }).map((_, index) => (
                <View
                  ...
                  style={[
                    styles.letterBox,
                    pattern[index] && pattern[index] !== '?' && styles.letterBoxFilled
                  ]}
                >
        ...
        <View style={styles.inputGroup}>
          ...
          <Text style={styles.hint}>Tap once to exclude • Tap twice to include • Tap three times to reset</Text>
          ...
          <View style={styles.legendContainer}>
            <View style={styles.legendItem}>
              ...
              <Text style={styles.legendText}>Excluded ({excludedLetters.length})</Text>
            </View>
            <View style={styles.legendItem}>
              <View style={[styles.legendBox, styles.includedKey]} />
              <Text style={styles.legendText}>Included ({includedLetters.length})</Text>
            </View>
          </View>
        </View>
            ...
            <View style={styles.wordsGrid}>
              {filteredWords.map((word, index) => (
                <TouchableOpacity
                  ...
                  onPress={() => {
                    Alert.alert('Word Selected', `"${word.toUpperCase()}" copied to clipboard!`);
                  }}
                >
